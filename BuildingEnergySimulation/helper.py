...
class Component():
    ...
    def __init__(self, building, connector_in=[], connector_out=[],
                 connection_in=None,):
    ...
    def out(self):
        ...
        if self.date == self.building.date:
            ...
            # loop over all needed inputs
            for connector in self.connector_in:
                values = []
                # loop over all connections
                for connection in self.connection_in:
                    # if the connector is part of the
                    # connection.connection_out, use it
                    if connector in connection.connector_out:
                        values.append(connection.out[connector])
                if len(values) > 0:
                    run_arg.update({connector: sum(values)})
                else:
                    if self.warn == 0:
                        print("No usable inputs {} found for {}".format(
                            connector, self.name))
                        self.warn = 1
    ...
    def run(self):
        raise NotImplementedError(
            "Run method needs to be created in children!")
